...
- **Push (webhook).** The app calls Agent-Int when something happens. Gmail pushes through Google Cloud Pub/Sub; Notion and Slack post directly to a delivery URL. Push triggers are near-instant.
- **Polling.** Agent-Int asks the app for new items on an interval. Google Forms is polled (default every 60s, configurable in the builder).
...
For the **push** apps, **save the workflow first** — the builder then reveals the **delivery URL** (`<APP_URL>/api/v1/webhooks/<triggerId>`) to register with the app. Where the app has an API for it (Slack's Manifest API, Gmail's `watch`), Agent-Int registers and renews the subscription for you; otherwise you paste the URL once. Each provider's page has the exact steps.
...
- **✓ Registered automatically** (green) — Agent-Int created the subscription through the app's API. Only **Gmail** and **Slack with a config token** reach this state.
