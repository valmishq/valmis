<script lang="ts">
	...
	let memory = $state<AgentMemoryEntry[]>([]);
	$effect(() => {
		agent = data.agent;
	...
	// ── Form state — initialized from agent data ──────────────────────────────
	let name = $state(data.agent.name);
	let description = $state(data.agent.description ?? '');
	let systemInstruction = $state(data.agent.systemInstruction ?? '');
	let avatarEmoji = $state(data.agent.avatarUrl ?? '🤖');
	let emojiPickerOpen = $state(false);
	let selectedCredentialIds = $state<Set<string>>(new Set(data.agent.credentialIds));
	let selectedModelConfigId = $state<string>(data.agent.modelConfigId ?? '');
	let selectedEmbeddingModelConfigId = $state<string>(data.agent.embeddingModelConfigId ?? '');
	...
	async function handleSubmit(e: SubmitEvent) {
		...
		if (!user) {
			setAlert({
				...
				message: 'You must be signed in to update an agent.',
				duration: 5000,
				show: true
			});
			isSaving = false;
		...
		try {
			const res = await api(`/agents/${agent.id}`, {
				method: 'PUT',
				body: JSON.stringify({
					ownerId: user.id,
					name,
					...
					modelConfigId: selectedModelConfigId || null,
					embeddingModelConfigId: selectedEmbeddingModelConfigId || null
				})
			});
...
<PageHeader title={agent.name} description="Edit agent configuration, credentials, and memory.">
	{#snippet actions()}
...
<form onsubmit={handleSubmit} class="space-y-6">
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description class="text-xs">Agent name and avatar.</Card.Description>
		</Card.Header>
				...
				<div class="space-y-2">
					{#each credentials as cred (cred.id)}
						...
						>
							...
							/>
							<!-- Service icon or fallback -->
							<div
								class="flex size-6 shrink-0 items-center justify-center rounded bg-muted text-muted-foreground"
							>
								{#if definitions.find((d) => d.id === cred.type)?.icon}
									<img
										src={definitions.find((d) => d.id === cred.type)?.icon}
										alt={cred.type}
	...
	<Card.Root>
		<Card.Header>
			...
			<Card.Description class="text-xs">
				Long-term memory entries stored by this agent. These are searchable at runtime via vector
				similarity.
			</Card.Description>
		</Card.Header>
		<Card.Content>
